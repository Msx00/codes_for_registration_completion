...
# ----------------------------- 数据项(强监督 + 一侧 CD) -----------------------------
class OneSidedCD(nn.Module):
    '''
    OneSidedCD 的 Docstring
    part 向 pred 的距离
    防止塌陷
    '''
...
class CD(nn.Module):
    ...
    def _one_sided_cd(self, src, dst):
        dist = torch.cdist(src, dst) ** 2  # (B, M, N)
        min_dist = torch.min(dist, dim=-1)[0]  # (B, M)
        return torch.mean(min_dist, dim=-1)  # (B,)
    
    def forward(self, pred, target):
        loss_target2pred = self._one_sided_cd(target, pred)  # (B,)
        loss_pred2target = self._one_sided_cd(pred, target)  # (B,)
...
def mse_loss(a, b):  # (B, N, 3)
    """Coordinate-wise MSE used by the existing supervised training loss."""
    return F.mse_loss(a.float(), b.float())


def rmse(a, b):  # (B, N, 3)
    """3D point RMSE between corresponding points."""
    difference = a.float() - b.float()
    squared_l2 = difference.square().sum(dim=-1)  # (B, N)
