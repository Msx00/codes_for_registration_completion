...
from liver2.losses.biomechanics import estimate_displacement_gradient
from liver2.losses.registration import point_rmse
...
def test(
    ...
    for batch in tqdm(loader, desc="Test", leave=True):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp and device.type == "cuda"):
            ...
            mae_value = F.l1_loss(pred, batch["gt_xyz"])
            ...
            # Compute negJ: proportion of points with negative Jacobian determinant
            disp = pred - batch["src_xyz"]
            grad_u = estimate_displacement_gradient(batch["src_xyz"], disp)
            eye = torch.eye(3, device=device, dtype=grad_u.dtype).view(1, 1, 3, 3)
            F_def = eye + grad_u.transpose(-1, -2)
            detF = torch.linalg.det(F_def.float())  # .float(): linalg.det has no Half implementation
            negJ = (detF < 0).float().mean()

        batch_size = batch["src_xyz"].shape[0]
        meters["mse"] += mse_value.item()
        meters["mae"] += mae_value.item()
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default=str(project_dir / "test_results"))
    ...
    ) or checkpoint_config.get("GIRNet_arch")
    if checkpoint_arch != "full2full_v3":
        parser.error(
            "checkpoint must use full2full_v3; "
            f"got architecture {checkpoint_arch!r}"
        )
    ...
    model = LiverV3Model(
        ...
        v3_feature_temperature=float(
            checkpoint_config.get("v3_feature_temperature", 1.0)
        ),
        v3_spatial_temperature=float(
            checkpoint_config.get("v3_spatial_temperature", 1.0)
        ),
        source_graph_k=int(checkpoint_config.get("source_graph_k", 16)),
    ).to(device)
